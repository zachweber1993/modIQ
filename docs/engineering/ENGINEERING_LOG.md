...
**Why this is being recorded, not adopted as a new mandatory stage:** one instance. Whether every future historically-derived procedure should include an explicit "search the full repository, not just the named evidence set, for an adjacent existing answer" step is left for a future Chief Architect session to decide, per this project's own standing bar of convergent evidence over a single favorable case.
