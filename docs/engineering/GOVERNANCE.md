...
ADR-0003 ("Assessment as the Aggregate Root," Accepted) lists `AssessmentReport` as an example of an Assessment-owned Runtime entity, alongside Evidence, Findings, and Recommendations. This reflected the platform's single-crate-era design at Sprint 1/2. It has not been true since the Reporting subsystem was given its own explicit crate boundary: `GOVERNANCE.md`'s own Reporting Crate Boundary Rule assigns `AssessmentReport` to `modiq-report`, generated by `AssessmentReport::generate(&assessment)` from `Assessment`'s already-public getters, never held or mutated by `Assessment` itself — confirmed directly against `crates/modiq-runtime/src/assessment/assessment.rs`, which holds no field, method, or dependency referencing `AssessmentReport` anywhere. `Architecture.md`'s own Platform Boundaries section independently corroborates this separation ("the architecture intentionally separates... Assessment execution... Reporting... Persistence"). This drift was never examined prior to Sprint 15 and is still repeated, unqualified, in `docs/engineering/PROJECT_HANDOFF_v1.1.md`'s current text.
...
Pending. Governance Initiation only (`docs/engineering/PROPOSAL_GOV-015.md`) — no Architecture Evaluation, Architectural Resolution, or Implementation Authorization has been performed. ADR-0003's still-valid core decision (Assessment as aggregate root) is explicitly out of this item's scope; only the `AssessmentReport` ownership example is in question. Per this project's standing discipline, a conflict with an Accepted ADR is resolved by governance decision — an amendment recorded explicitly, or a new superseding ADR — never by silent reinterpretation.
