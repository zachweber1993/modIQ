...
**Subsequently addressed:** the `AssessmentReport` ownership finding above was taken up as GOV-015 and resolved via ADR-0011 (`docs/adrs/0011-assessmentreport-ownership-correction.md`), which corrects ADR-0003's example without disturbing its aggregate-root decision. See the GOV-015 entry below.
...
Following `docs/engineering/GOV002_ARCHITECTURE_EVALUATION.md` (GOV-002 Architecture Evaluation, Finding 3) — Sprint 16 Governance Initiation (`docs/engineering/PROPOSAL_GOV-015.md`)

Description

ADR-0003 ("Assessment as the Aggregate Root," Accepted) lists `AssessmentReport` as an example of an Assessment-owned Runtime entity, alongside Evidence, Findings, and Recommendations. `GOVERNANCE.md`'s own Reporting Crate Boundary Rule assigns `AssessmentReport` to `modiq-report`, generated by `AssessmentReport::generate(&assessment)` from `Assessment`'s already-public getters, never held or mutated by `Assessment` itself — confirmed directly against `crates/modiq-runtime/src/assessment/assessment.rs`, which holds no field, method, or dependency referencing `AssessmentReport` anywhere. `Architecture.md`'s own Platform Boundaries section independently corroborates this separation. Sprint 17's Architecture Evaluation (`docs/engineering/GOV015_ARCHITECTURE_EVALUATION.md`) corrected an earlier assumption from GOV-002's own evaluation: this is not the architecture having evolved past ADR-0003 — `modiq-report` existed as a crate separate from `modiq-runtime` concurrent with ADR-0003's own date (2026-07-16), confirmed directly against `CrateRoadmap.md`'s revision history and `CHANGELOG.md`'s own Sprint 1 entry. The actual cause is that ADR-0007, accepted one ADR later, established a precise technical definition of "ownership" that ADR-0003's own earlier, looser example list was never checked against.

Question

Does ADR-0003's inclusion of `AssessmentReport` as an Assessment-owned entity still accurately describe the platform's architecture? If not, is the correct reconciliation an in-place ADR-0003 amendment or a new ADR that supersedes ADR-0003's specific claim without disturbing its still-valid core decision (`Assessment` as the Runtime domain's sole aggregate root, which this item does not question)? Does the correction additionally require reconciling `PROJECT_HANDOFF_v1.1.md` or any other living document found to repeat ADR-0003's original framing?

Resolution

**Resolved.** Architecture Evaluation and Architectural Resolution (`docs/engineering/GOV015_ARCHITECTURE_EVALUATION.md`, Section 6) determined: (1) the discrepancy is an editorial imprecision in ADR-0003's own drafting, not an architectural inconsistency — the actual architecture has never been internally inconsistent; (2) ADR-0007 supersedes ADR-0003's terminology in substantive effect (its precise ownership definition is what should govern) without formally superseding the document itself; (3) `docs/adrs/README.md`'s own stated policy — already applied once, to ADR-0002's own uncorrected numbering defect — requires a new superseding ADR rather than in-place amendment; (4) the remaining discrepancy is purely documentary, not architectural — no behavior, boundary, or ownership relationship needs to change. `PROJECT_HANDOFF_v1.1.md`'s current text was found to trace to `DataModel.md`'s own independent "generates" framing, not a repetition of ADR-0003's "owned" language, and likely does not itself require a substantive change (working assumption, to be re-checked once a superseding ADR's text exists).

**Recommended follow-up, described only, not authorized by this resolution:** prepare a proposal for a superseding ADR (`PROPOSAL_ADR-0011.md`, or the next available number), addressing only ADR-0003's `AssessmentReport` inclusion and preserving its still-valid core decision. Per this project's own governance discipline, the proposal is not itself an authoritative decision — it exists solely for Chief Architect review; upon approval it becomes the basis for the superseding ADR, which may then proceed through repository reconciliation. Each of proposal preparation, ADR creation, and reconciliation requires its own explicit authorization under the repository's stage-gated workflow; none is authorized by this resolution.

**Status update:** this recommended follow-up has since been carried out. `PROPOSAL_ADR-0011.md` was prepared and approved (Sprint 18), and **ADR-0011** (`docs/adrs/0011-assessmentreport-ownership-correction.md`) was created and accepted (Sprint 19), correcting ADR-0003's `Assessment Report` example exactly as this Resolution anticipated. ADR-0003 itself remains unmodified. Repository reconciliation of the surrounding governance and tracking documents followed at Sprint 21.
