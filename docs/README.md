...
- `docs/platform/DashboardAndConsole.md` — the third approved governed topic document. Refines Console (the persistent interaction shell) and Dashboard (its default region); references Workspace only to establish containment, without redefining it. Resolves no Architectural Decision Deferred. Approved.
- `docs/platform/WayfindingAndSearch.md` — the fourth approved governed topic document. Refines Wayfinding (movement among Organizations, Projects, and Assessments) and Search (locating them); establishes that visibility remains Identity & Access's alone and that neither Wayfinding nor Search creates or mutates Platform state. Resolves no Architectural Decision Deferred. Approved.
- `docs/platform/Notifications.md` — the fifth approved governed topic document. Refines Notification as the Platform Architecture responsible for communicating already-existing Platform state; establishes that Notifications never create, mutate, authorize, present, or navigate Platform state. Resolves the final item of `PlatformSpecification.md`'s Architectural Decisions Deferred (Notification addressing remains User-only, constitutionally User-owned — an Organization-wide occurrence is represented by distinct, User-owned Notifications, one per Member). Approved.
- Remaining governed topic documents (`SettingsAndBilling.md`, `PlatformSurfaces.md`) have not yet been drafted.