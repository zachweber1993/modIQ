...
**Platform Architecture 1.0 is Frozen, effective 2026-07-30** — the six documents below are now permanent; any future change to any of them is a governed amendment, not ordinary editing.

- `docs/platform/PlatformSpecification.md` — the constitutional foundation of the Platform Architecture lineage. Frozen.
- `docs/platform/IdentityAndAccess.md` — the first governed topic document. Refines Organization, User, Membership, and API Client; defines the Role attribute of Membership (Owner, Member); resolves the Project ownership root and API Client access scope items from `PlatformSpecification.md`'s Architectural Decisions Deferred. Frozen.
- `docs/platform/ProjectsAndUploads.md` — the second governed topic document. Refines Project and Upload; resolves the Project mandatory-or-auto-created item from `PlatformSpecification.md`'s Architectural Decisions Deferred (auto-created). Frozen.
- `docs/platform/DashboardAndConsole.md` — the third governed topic document. Refines Console (the persistent interaction shell) and Dashboard (its default region); references Workspace only to establish containment, without redefining it. Resolves no Architectural Decision Deferred. Frozen.
- `docs/platform/WayfindingAndSearch.md` — the fourth governed topic document. Refines Wayfinding (movement among Organizations, Projects, and Assessments) and Search (locating them); establishes that visibility remains Identity & Access's alone and that neither Wayfinding nor Search creates or mutates Platform state. Resolves no Architectural Decision Deferred. Frozen.
- `docs/platform/Notifications.md` — the fifth governed topic document. Refines Notification as the Platform Architecture responsible for communicating already-existing Platform state; establishes that Notifications never create, mutate, authorize, present, or navigate Platform state. Resolves the final item of `PlatformSpecification.md`'s Architectural Decisions Deferred (Notification addressing remains User-only, constitutionally User-owned — an Organization-wide occurrence is represented by distinct, User-owned Notifications, one per Member). Frozen.
- `docs/platform/SettingsAndBilling.md` — a governed topic document, drafted after the freeze and unaffected by it. Refines Settings as a non-owning interaction region presenting and surfacing management action over Organization- and User-scoped Platform state — Account Settings (Membership) and Subscription & Billing (Subscription plan selection, invoice visibility, API Client). Resolves no Architectural Decision Deferred (all four already resolved). Approved.
- `docs/platform/PlatformSurfaces.md` was retired from `PlatformSpecification.md`'s Governs list on 2026-08-01 — a dedicated Scope Determination and Historical Determination found it was never assigned an architectural responsibility; the platform experience layer it could only plausibly have named is already completely governed by `DashboardAndConsole.md`, `WayfindingAndSearch.md`, and `SettingsAndBilling.md`. The Platform Architecture lineage is now six governed documents plus its constitutional foundation, with no undrafted document remaining.